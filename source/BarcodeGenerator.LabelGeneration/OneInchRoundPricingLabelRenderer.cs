...
/// designed to render pricing labels using the <see cref="LabelTemplateType.OneInchRound"/> template.
/// It encapsulates the logic required to draw the label onto a graphical surface within defined bounds.
/// </remarks>
public sealed class OneInchRoundPricingLabelRenderer : ILabelRenderer {
    /// <summary>
    /// Gets the template type of the label rendered by the <see cref="OneInchRoundPricingLabelRenderer"/>.
    /// </summary>
    /// <value>
    /// A <see cref="LabelTemplateType"/> value indicating the specific template type used for rendering.
    /// </value>
    /// <remarks>
    /// This property returns <see cref="LabelTemplateType.OneInchRound"/>, which represents a label template
    /// designed for one-inch round pricing labels.
    /// </remarks>
    public LabelTemplateType TemplateType => LabelTemplateType.OneInchRound;
    ...
    public void Render(IPrintableLabel label, Graphics graphics, Rectangle bounds) {
        ...
        if (label is not PricingLabel pricingLabel) {
            throw new ArgumentException(
                $"{nameof(OneInchRoundPricingLabelRenderer)} requires a {nameof(PricingLabel)}",
                nameof(label)
            );
        }
        ...
        if (string.IsNullOrWhiteSpace(conditionText)) {
            var priceOnlyBounds = new RectangleF(safeBounds.Left + (bounds.Width * 0.04f),
                safeBounds.Top - (bounds.Height * 0.15f),
                safeBounds.Width,
                safeBounds.Height);
        ...
        var priceBounds = new RectangleF(safeBounds.Left + (bounds.Width * 0.04f),
            safeBounds.Top - (bounds.Height * 0.15f),
            safeBounds.Width,
            safeBounds.Height);
        ...
        var conditionBounds = new RectangleF(
            safeBounds.Left + (safeBounds.Width * conditionHorizontalOffset),
            safeBounds.Top + (safeBounds.Height * 0.47f),
            safeBounds.Width * (1.0f - conditionHorizontalOffset),
            safeBounds.Height * 0.22f);
